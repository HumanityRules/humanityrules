...
An AI personal assistant powered by [OpenClaw](https://github.com/openclaw/openclaw), configured for enterprise deployment via DevOps Hero. Supports Slack integration out of the box.
...
- **Secrets** — `ANTHROPIC_API_KEY`, `OPENCLAW_GATEWAY_TOKEN`, and Slack tokens are managed via DOH secrets.
...
For DOH enterprise deployments, the manual pairing step doesn't make sense — DOH already governs access through ABAC, approval workflows, and VPC network isolation. We disable it with:
...
This makes the gateway rely on token auth only (no per-device approval). The `dangerously` prefix is OpenClaw's warning for self-hosters; behind DOH's ALB + governance layer, this is the correct setting.

**Future option: `trusted-proxy` mode.** When DOH injects user identity headers at the ALB (e.g., via WorkOS/AuthKit claims), OpenClaw can consume them directly via `gateway.auth.mode: "trusted-proxy"` with `gateway.auth.trustedProxy.userHeader` pointing to the identity header. This would give per-user audit trails inside OpenClaw itself. Not implemented yet — requires DOH to forward identity headers to deployed apps.
