"""Historical reference: the Hermes Docker AppTemplates.

This file is a verbatim snapshot of the helpers and AppTemplate definitions
that were removed from `devopshero_app/management/commands/seed_app_templates.py`
when the `hermes-docker-personal` and `hermes-docker-slack` templates were
retired. The matching `template_repos/` source trees moved to:

  - `template_repos/deprecated/hermes_docker_agent/`  — the AppTemplate-bound
    Hermes build (source for `_HERMES_CONTAINER_BASE.source_repo_path`).
  - `template_repos/deprecated/doh_dind/`             — the Docker-in-Docker
    sidecar image referenced by `_DOCKER_DIND_CONTAINER`.

This file is NOT imported anywhere. It is kept as reference for the shape of
the deprecated multi-container templates (DinD sidecar + Hermes + policy proxy
or sidecar-mcp). It will not run as a standalone module — it references a few
symbols that still live in the active `seed_app_templates.py`:

  - `_HERMES_LLM_VARS`                (active)
  - `_HERMES_TAVILY_VAR`              (active; spread into `_HERMES_CREDENTIAL_VARS` below)
  - `_HERMES_POLICY_PROXY_CONTAINER`  (active; used by the surviving `hermes-personal` template too)

Everything else below was dropped from the active module along with the two
template definitions. See
`devopshero_app/migrations/0057_remove_hermes_docker_templates.py` for the
data cleanup that removed the matching DB rows.
"""

# -- Removed helpers (in original order) -----------------------------------
...
# template_repos/deprecated/doh_dind/ for the Dockerfile, entrypoint, and snapshotter.
_DOCKER_DIND_CONTAINER = {
    "name": "docker-dind",
    "image_source": "prebuilt",
    "ecr_repo": "doh-dind",
    # Image tag DOH expects in doh/{env_slug}/doh-dind. Bump when we ship a new
    # snapshotter or entrypoint in template_repos/deprecated/doh_dind/ and push via
    # `doh_build_prebuilt_image --source-dir template_repos/deprecated/doh_dind --ecr-repo doh-dind --tag X.Y.Z`.
    "version": "0.2.5",
...
_HERMES_CONTAINER_BASE = {
    ...
    "source_repo_path": "deprecated/hermes_docker_agent",
