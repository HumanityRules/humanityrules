#!/usr/bin/env bash
# GitHub environment detection helper (HUMR sandbox variant).
#
...
#   GITHUB_TOKEN    - placeholder credential, swapped in flight by the proxy
#   GH_USER         - GitHub username of the connected user
...
#
# Auth is platform-managed (see the github-auth skill). This helper only
# detects whether the user's GitHub connection is live — it never sets up
# credentials.
...
[ "$GH_AUTH_METHOD" = "none" ] && echo "⚠ GitHub not connected — ask the user to open the Integrations panel and click Connect on GitHub"
