"""
Policy-proxy auth Lambda — OAuth with Okta + session JWT minting.

Triggered by ALB via a host-based listener rule on auth.<env-domain>. See
docs/policy_proxy_design.md for the overall flow.
...
@dataclass(frozen=True)
class PolicyProxyAuthConfig:
    oidc: OidcConfig
    jwt_key: JwtKeyConfig


_cached_config: PolicyProxyAuthConfig | None = None
...
def load_policy_proxy_auth_config() -> PolicyProxyAuthConfig:
    global _cached_config
    if _cached_config is not None:
        return _cached_config
    arn = os.environ["DOH_POLICY_PROXY_AUTH_CONFIG_SECRET_ARN"]
    data = _load_secret_json(arn)
    oidc_data = data["oidc_config"]
    jwt_data = data["jwt_key"]
    _cached_config = PolicyProxyAuthConfig(
        oidc=OidcConfig(
...
def auth_base_url() -> str:
    # The URL the policy proxy redirects clients to. Registered as a single callback
...
def _exchange_code_for_userinfo(code: str, redirect_uri: str) -> dict:
    cfg = load_policy_proxy_auth_config().oidc
