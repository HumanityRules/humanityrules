# Policy-Proxy Auth Lambda

One per DOH environment. Handles the Okta OAuth flow and mints `doh_session` JWTs that policy proxies verify per request. Fronted by the env ALB via a host-based listener rule on `auth.<env-domain>`.
...
- `GET /.well-known/jwks.json` — serve the env's public key for policy-proxy verification.

## Runtime model

Triggered by ALB (not API Gateway). The Lambda is configured with a single Secrets Manager ARN passed as an env var:

- `DOH_POLICY_PROXY_AUTH_CONFIG_SECRET_ARN` — JSON blob with both the OIDC config and the env's JWT signing keypair:
