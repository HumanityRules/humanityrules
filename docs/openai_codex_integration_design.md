...
- `provider_auth_marker.py` writes the placeholder provider block on connect, clears it on disconnect (Codex uses the agent's locked/atomic `_save_codex_tokens`; Nous uses upstream `persist_nous_credentials`). Presence — not value — is the picker's signal; the picker's cache keys on a semantic hash of auth.json that includes this block, so add/remove flips it and the next `/api/models` rebuild shows/hides Codex **without a WebUI restart**.
- Run via `runuser -u hermeswebui` from the (root) broker, at the same two hook points that invalidate the provider TLS cache (device connect; `disconnect_route`). Keeps auth.json owned 0600 by the gateway user.
