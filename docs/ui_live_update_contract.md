...
- `humanityrules_app/templates/humanityrules_app/apps/_app_deployment_section.html` (the app page's single clock: its 10s poll OOB-swaps `_app_removal_banner.html`, `_app_detail_actions.html`, and `_app_deployment_history.html`; the Overview tab button refetches the same endpoint on click, which also restarts the 10s delay)
