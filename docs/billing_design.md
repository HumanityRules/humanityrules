...
4. Renewal grants: the invoice-paid webhook writes an `expiry` entry writing off the previous period's balance whatever its sign, then the `grant` entry (`grant:{subscription_id}:{period_start}`) — every paid period starts at exactly the plan's grant. A positive write-off is unused entitlement expiring; a negative one forgives grace overspend, bounded by the enforcement floor (§7.1.3) at 5% of a month's grant. One grant alive at a time — pooled balance and grant coincide, which is what makes grant lots deferrable.
...
8. `customer.subscription.created`, `.updated`, and `.deleted` are newest-state-wins by the event envelope's Stripe `created` timestamp. A strictly older event is retained but not applied; equal timestamps apply normally. Invoice events are deliberately outside this ordering guard, and their period grants remain idempotent.
