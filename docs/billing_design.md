...
8. `customer.subscription.created`, `.updated`, and `.deleted` are newest-state-wins by the event envelope's Stripe `created` timestamp. A strictly older event is retained but does not replace subscription facts; equal timestamps apply normally. Invoice events are deliberately outside this ordering guard. Reconciliation still runs after either family, and the existing `grant:{subscription_id}:{period_start}` / `expiry:{subscription_id}:{period_start}` keys make repeated reconciliation and paid-invoice delivery harmless.
