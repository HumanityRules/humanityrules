...
2. **Operator:** $29/month, 2,000 credits/month, always-on, 1 agent.
3. **Team:** $19/agent/month — published on the billing page but still sales-led, no self-serve path. Unlimited agents, customer cloud, bring-your-own model.
...
1:1 with Organization. Stripe customer id, subscription id, nullable status, current period start/end, and `latest_paid_period_start`. A pure Stripe-fact mirror mutated only by webhook handlers: subscription events supply subscription state; `invoice.paid` alone supplies the paid-period fact and may create a statusless partial mirror when it arrives first. No Invoice or PaymentMethod models — Stripe's portal owns those.
...
3. Threshold: block at −5% of the monthly grant (−100 credits for Operator), not at zero. Delayed events and in-flight turns make exact-zero enforcement dishonest without reservation machinery; a fixed negative floor is simple and truthful. The write-off at renewal absorbs any grace overspend (§8.4), so a period always begins at the full grant.
...
3. The webui-extension renders a credits card at the bottom of the sidebar (Lovable-style, a small rounded card): "Credits {remaining}/{grant}" with an Upgrade link beside it, then the plan name, then the renewal date. Two states — quiet normally, drawing the eye at exhaustion; there is no separate running-low state and no meter bar. The Upgrade link (to the CP billing page) is always present. Negative balances display as zero: enforcement legitimately runs to −5%, and a negative number in the sidebar reads as a bug. The renewal line appears only when the snapshot carries a renewal date; trial's one-time grant never does, and none exists until subscriptions ship. The card renders nothing at all until the broker has a usable snapshot.

## 8. Stripe and plan lifecycle

The subscription guards follow from two axioms rather than five unrelated
delivery exceptions. A duplicate `customer.subscription.created` is the one
case that violates an axiom outright; its guard is a fail-loud tripwire for
that violation, while the normal newest-state-wins path still picks one
recoverable mirror state for it, same as any other subscription event:

1. **One followed subscription.** The mirror follows exactly one subscription
   until that subscription reaches terminal state. Ordinary state and payment
   events for another id have no standing against the live subscription the
   mirror already follows. A second `customer.subscription.created` while that
   subscription is live is not ordinary delivery; it violates this invariant
   and must surface the manual-intervention anomaly below.
2. **One winning state.** Among subscription events for the followed
   subscription, the newest Stripe envelope timestamp wins. When timestamps
   tie, `canceled` wins over a status that would resurrect the same
   subscription.

1. Stripe Checkout for Operator signup; Stripe customer portal for card management and cancellation. Monthly billing only.
2. Webhooks handled: subscription created, renewed (invoice paid), canceled, payment failed. Every handled type first requires `data.object` to be a dictionary; a missing or differently shaped object raises inside the per-event transaction, rolling back retention and effects so Stripe receives a failure and can retry instead of permanently deduplicating an unapplied event. This outer-object contract applies to both subscription and invoice events, while individual invoice facts remain sparse and optional. Each handler records only the facts its event owns in `BillingSubscription`, then one reconciliation step derives all consequences from the combined mirror. Subscription events exclusively own status and also own periods, customer/subscription ids, and cancellation timestamps. They are complete snapshots: customer id and subscription id must be non-empty strings, and created/updated events must also carry a non-empty status (`deleted` derives terminal `canceled` from its event type). Those required fields are validated before any stale/superseded delivery guard. `invoice.paid` owns `latest_paid_period_start` and also writes only the periods and identity it actually carries, but never status — if it arrives before the first subscription event it creates a statusless partial mirror instead of assuming `active`. `invoice.payment_failed` owns no mirror facts but remains handled and retained before reconciliation when its outer object is valid.
3. Reconciliation runs after every handled event and first derives `Organization.plan` from mirrored status: active, trialing, or past due → `operator`; canceled or payment failed past retries → `trial`, with an actual Operator→Trial transition also restarting the trial credits (item 11). Plan is the entitlement source of truth and legitimately diverges from payment (friends-and-family orgs, enterprise invoiced outside Stripe, comps) — hand-set plans remain guarded from Stripe-driven changes. Only after that transition, when the resulting plan is Operator and `latest_paid_period_start` matches the mirrored current period, reconciliation attempts the period grant. The match prevents a new subscription id from reusing the previous subscription's paid fact. Whichever event completes the matching state and payment facts triggers the consequence, so same-second `invoice.paid` / `customer.subscription.created` delivery order is irrelevant.
4. Renewal grants: reconciliation writes an `expiry` entry writing off the previous period's balance whatever its sign, then the `grant` entry (`grant:{subscription_id}:{period_start}`) — every paid period starts at exactly the plan's grant. The paid fact comes only from `invoice.paid`, never from the subscription object's period fields: Stripe may advance those fields on a failed renewal, and treating that advance as payment would grant free credits. A positive write-off is unused entitlement expiring; a negative one forgives grace overspend, bounded by the enforcement floor (§7.1.3) at 5% of a month's grant. One grant alive at a time — pooled balance and grant coincide, which is what makes grant lots deferrable. The deliberately manual, read-only `humr_billing_verify` check considers only active mirrors for Operator organizations: `past_due` means nobody paid, and `canceled` means no live subscription exists even if the plan was hand-set. It reports a possible permanently lost `invoice.paid` when the mirror has both period facts, `latest_paid_period_start` is strictly older than `current_period_start`, and the mirror's `updated_at` has remained unchanged for more than one day. The mirror timestamp is a conservative grace anchor for Stripe's normal renewal delay; any intervening mirror write postpones the finding. It also reports an active mirror whose paid period equals its current period but whose `grant:{subscription_id}:{period_start}` ledger entry is missing. Because the mirror keeps no period history, either signature disappears after the next successful renewal; despite being manual-only, the command must run at least once per monthly billing cycle.
5. Payment failure: Stripe smart retries run their course; downgrade on final failure. No custom dunning.
6. Trial→Operator upgrade: Checkout produces subscription-state and paid-period webhook facts in either order; the event that completes the mirror lets reconciliation flip the plan and post the monthly grant. The 500-credit trial grant is a one-time entry at org creation.
7. Every signature-verified webhook enters one retention-and-apply transaction. Successful events are retained in full, including unhandled types, which are recorded but produce no effects; a malformed handled event that raises is not retained at all. Stripe event id is the deduplication key, so a retained event means either its effects committed or its type is one HumR doesn't act on. Redelivery skips processing entirely.
8. The two axioms produce the five delivery guards:

   1. A `customer.subscription.created` naming a second subscription while the mirror follows a different, non-canceled one violates the one-followed-subscription axiom, so it cannot pass silently: the guard logs both subscription ids and the organization as a manual-intervention error. Recovery behavior is unchanged: the event still enters the normal newest-state-wins path, and the tripwire neither refuses nor cancels either subscription. If the mirror ends up following the duplicate rather than the keeper, manual intervention includes correcting the mirror as well as resolving the duplicate in Stripe.
   2. An updated or deleted event naming a different subscription from a non-canceled, non-null mirror is for a superseded id and cannot overwrite the live followed subscription, so its facts are skipped.
   3. An `invoice.paid` naming such a superseded id is skipped for the same reason. Otherwise it may contribute its invoice-owned facts even before subscription state arrives.
   4. A subscription event whose Stripe `created` timestamp is strictly older than the followed subscription's newest mirrored event is retained but cannot move the mirror backward, so its facts are skipped.
   5. Equal timestamps normally apply. For the same subscription id, when the mirror is already `canceled`, an equal-timestamp event carrying any other status is retained but cannot resurrect it; the tie is logged and resolved in favor of `canceled`.

   A strictly newer subscription event always applies normally, including a real resumption or recovery. Invoice events are deliberately outside the timestamp-ordering guard. Reconciliation still runs after either event family, and the existing `grant:{subscription_id}:{period_start}` / `expiry:{subscription_id}:{period_start}` keys make repeated reconciliation and paid-invoice delivery harmless.
9. The subscription mirror stores `cancel_at` and `canceled_at`, written verbatim including nulls so a resumed cancellation clears them. `cancel_at` is the pending-cancellation signal and the plan end date: on the pinned dahlia API versions a portal cancel-at-period-end arrives with `cancel_at` set and `cancel_at_period_end` still false, so the legacy boolean is deliberately not stored. The actual downgrade still waits for `customer.subscription.deleted` — `cancel_at` is displayed, never compared against the clock. While pending, the CP billing page shows the plan end and points to Manage billing for resumption; the broker entitlement snapshot omits its renewal date.
10. Checkout success returns to `/settings/billing/` with Stripe's substituted Checkout Session id in `checkout_session_id`; cancellation returns without it. While that marker is present and the organization remains on Trial, the page shows activation in progress without an Upgrade action and self-polls until the webhook changes the plan. The marker is logged for diagnosis and is otherwise an untrusted display hint: no Stripe lookup occurs, and only webhooks change plans or credits.
11. Each Operator→Trial reconciliation flip restarts the trial balance: under one balance lock, an `expiry` writes off the full balance whatever its sign, then a `grant` sets it to the effective Trial grant (500 unless the organization overrides it). The keys are `expiry:trial-reset:{stripe_event_id}` and `grant:trial-reset:{stripe_event_id}`. Stripe event ids are globally unique and already deduplicated when the webhook is retained, while checking the grant key under the lock also makes reapplying the same flip harmless. A later flip caused by a different event receives its own reset even when the subscription id is unchanged. This is separate from and does not change the signup key `grant:trial:{org_id}`.
12. Hand-setting a subscribed organization's plan to `team` or `enterprise` does not touch Stripe: the transition guard stops Stripe-driven plan changes, reconciliation stops posting Operator grants, and the customer's $29 subscription keeps charging for nothing. Moving a subscribed organization to a sales-led plan therefore includes canceling its Operator subscription in the Stripe dashboard as part of the same manual step.
