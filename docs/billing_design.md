...
2. Webhooks handled: subscription created, renewed (invoice paid), canceled, payment failed. Each handler records only the facts its event owns in `BillingSubscription`, then one reconciliation step derives all consequences from the combined mirror. Subscription events exclusively own status and also own periods, customer/subscription ids, and cancellation timestamps. An updated or deleted event naming a different subscription from a non-canceled, non-null mirror is skipped, as is an `invoice.paid` for such a superseded id. Otherwise `invoice.paid` owns `latest_paid_period_start` and also upserts its carried periods and identity, but never status — if it arrives before the first subscription event it creates a statusless partial mirror instead of assuming `active`. `invoice.payment_failed` owns no mirror facts but remains handled and retained before reconciliation.
3. Reconciliation runs after every handled event and first derives `Organization.plan` from mirrored status: active, trialing, or past due → `operator`; canceled or payment failed past retries → `trial`, with an actual Operator→Trial transition also restarting the trial credits (item 12). Plan is the entitlement source of truth and legitimately diverges from payment (friends-and-family orgs, enterprise invoiced outside Stripe, comps) — hand-set plans remain guarded from Stripe-driven changes. Only after that transition, when the resulting plan is Operator and `latest_paid_period_start` matches the mirrored current period, reconciliation attempts the period grant. The match prevents a new subscription id from reusing the previous subscription's paid fact. Whichever event completes the matching state and payment facts triggers the consequence, so same-second `invoice.paid` / `customer.subscription.created` delivery order is irrelevant.
...
7. Every signature-verified webhook is retained in full, including unhandled types. Stripe event id is the deduplication key; recording and applying share one database transaction, so a retained event means all of its effects committed. Redelivery skips processing entirely.
8. `customer.subscription.created`, `.updated`, and `.deleted` are newest-state-wins by the event envelope's Stripe `created` timestamp. A strictly older event is retained but does not replace subscription facts. Equal timestamps normally apply, except that for the same subscription id, when the mirror is already `canceled`, an equal-timestamp event carrying any other status is retained but cannot replace the terminal cancellation; the tie is logged and resolved in favor of `canceled`. A strictly newer event always applies normally, including a real resumption or recovery. Invoice events are deliberately outside this ordering guard. Reconciliation still runs after either family, and the existing `grant:{subscription_id}:{period_start}` / `expiry:{subscription_id}:{period_start}` keys make repeated reconciliation and paid-invoice delivery harmless.
