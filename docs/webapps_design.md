...
- **Readiness probe is an HTTP-shaped bind check.** process-compose has only `exec` and `http_get` probes (no native `tcp_socket`), so the CLI emits an `exec` probe that opens `/dev/tcp/127.0.0.1/<port>` and sends a minimal `HEAD / HTTP/1.1` request. It briefly reads and ignores the response; success is still conditioned on connect/write, not on a `200` response. The generated probe starts after 5 seconds, checks every 10 seconds, and allows 6 consecutive failures before process-compose stops/restarts the app. That gives slow-start apps about a minute to bind the port while still surfacing genuinely broken processes. The probe tells you the app accepted an HTTP-shaped connection; it doesn't tell you the app is *correct*. That's the bare minimum we want for `webapps start` to claim success.
