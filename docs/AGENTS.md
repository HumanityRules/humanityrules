...
- **`docs/cost_per_ha.md`** — Concise monthly cost estimate for one HA in the shared sandbox, including allocated `r8g.large` infrastructure, Enhanced Container Insights, storage, and usage-dependent unit costs; shared environment infrastructure is listed separately.
...
- **`docs/app_removal_data_cleanup_audit.md`** — Audit of what app removal destroys (now unconditionally: infra, EFS + host data, secrets, `app-name=` policies) and the one thing it keeps: `IntegrationUserCredential` rows have no FK to `App` and are retained on purpose, per-user. Open when reworking app removal, debugging phantom reconnects after recreating an app, or auditing GDPR. Snapshot 2026-05-24.
