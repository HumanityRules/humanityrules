...
- **ARN → app mapping is FORWARD.** `identity.arn` role = `humr-{env.slug}-{app.slug}-task-role`, truncated to 64 chars. `app_name = app.slug` is set in `services/jobs/app_config_builder.py` — the deploy path uses the slug, not `app.name`. Env and app slugs both contain hyphens, so reverse-parsing the role name is ambiguous. Instead, for each environment the app is deployed to, **build the expected role name and filter the query on it**.
...
**Prior art for inspiration, but no need to copy:** `_get_aws_session_for_environment(env)` in `services/infra_customer/iam_utils.py` (assumed-role session for querying a customer account).
