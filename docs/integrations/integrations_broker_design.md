...
- **Customer env supervisor → HUMR control plane:** the per-app bearer token (`HUMR_APP_BEARER`) that also authenticates the policy-proxy's PDP calls. App-scoped: HUMR resolves the App from the token and derives the environment, org and owning user (the app's `owner` tag) from it — the request carries no identity fields. The supervisor has the bearer; the sandboxed agent does not.
...
- **`POST /integrations/refresh_all`** — explicit user refresh: reload the MCP catalog and drop-and-resync every TLS-intercept provider (cooldown-gated by the broker's credentials service; 429 if called too soon). The WebUI re-fetches `GET /integrations` after a successful refresh.
- **`POST /integrations/tls_intercept/<provider>/disconnect`**, **`POST /integrations/tls_intercept/<provider>/setup-session`**, **`POST /integrations/tls_intercept/<provider>/resync`**, **`POST /integrations/tls_intercept/<provider>/device/start|status|cancel`** — TLS-intercept providers (Google, GitHub, Telegram, Slack, device-flow LLMs). Registered directly on the broker router.
...
- **`GET /integrations/merge/connector-status`**, **`POST /integrations/merge/link-token`**, **`POST /integrations/merge/disconnect`** — Merge passthroughs that forward to HUMR with the app bearer attached. See `merge_integration_design.md`.
...
- **`humr_client.py`** — the outbound JSON client for HUMR's bearer-authenticated endpoints, holding the control-plane URL and the app bearer. It still knows the owner/app identity for the WebUI status card, but sends payloads as given — identity is derived from the bearer on HUMR. Token refresh, device-flow completion, disconnect, and vault setup sessions all go through it. (`mcp_merge_backend` is the one component that keeps its own copy of the bearer, for its HUMR relay calls.)
...
- **`tls_provider_catalog.py`** — static data only. The credential wire-behavior dataclasses, the `TlsProviderSpec` entries, and the registries built from them. The only file a provider that uses an existing wire behavior needs.
...
- **`tls_credential_state.py`** — `CredentialStateStore`: a slug-keyed cache of injectable secrets, the HUMR refresh protocol, and cache-independent per-provider connection state. It knows neither hosts nor provider specs and returns copied state snapshots for `TlsInterceptRuntime` to join with the catalog.
...
- **The proxy hot path.** An intercepted request whose cached secret is missing, expired, or within `REFRESH_LEAD_SECONDS` (300) of expiry refetches that one slug before going upstream. Fetch and apply both happen under the store's single lock, which is what makes "a parked fetch wrote past a concurrent token drop" structurally impossible.
- **A known state change.** Connect, disconnect, vault save, or device-flow completion drops that slug's cached token and refetches only it. Fanning out to every slug on each connect would ask HUMR about providers the user has never connected.
...
- **App bearer token** — the app's own Secrets Manager bag in the customer account (`humr/{env}/{app}/secrets`, key `HUMR_APP_BEARER`), with only the hash stored on HUMR. Same custody pattern as the policy proxy's token — it *is* the same token.
...
- **`credential_wire_behavior`** — how an intercepted request identifies a managed credential and where its named secret is written. Every `secret_name` is also a control-plane response-schema contract: the provider's `refresh_outcome` must return that exact key in `secrets`. See below.
...
- **`AlwaysInjectHeaders`** — every intercepted request uses HUMR's managed credential. Its `header_injections` name each required secret, destination header, and header-value format: bearer for Google, X, and Nous; `basic_x_access_token` for GitHub git smart-HTTP; bearer plus raw `ChatGPT-Account-ID` for Codex.
- **`HeaderPlaceholder`** — an empty credential header is anonymous pass-through; a recognized placeholder selects its named secret; a foreign credential is refused. `placeholder_by_secret_name` states the control-plane secret key → sandbox placeholder mapping directly. Placeholder headers support only raw and bearer formats; Basic is valid only for unconditional header injection. `remove_authorization` and `reject_authorization_when_placeholder_missing` make custom-header exclusivity explicit per provider.
- **`UrlCredentialPlaceholder`** — the request URL must contain the configured credential placeholder, which is replaced by its named secret. Telegram's `/bot{token}/` and `/file/bot{token}/` paths use this behavior. A missing placeholder is refused because that host has no anonymous surface; `remove_authorization` explicitly records whether a separate Authorization header is stripped.

`plan_injection` runs before the token-store lookup and returns one `InjectionPlan`, `None` for anonymous pass-through, or `SecretSelectionError` for a refused request. The plan retains the selected secret names, wire destinations, and explicit Authorization policy across the asynchronous lookup. `apply_injection_plan` then performs only those recorded substitutions; it does not branch on provider behavior.

Two request refusals are worth knowing. Both raise `SecretSelectionError` and receive a 400 rather than being forwarded:
...
A managed credential missing any secret named by its injection plan is not a sandbox request error. It raises `CredentialContractError`, logs the missing control-plane key, and returns a generic 502 to the sandbox.
...
- **Customer app compromised.** The attacker gets that app's bearer, which can mint access tokens for the app's owner only — bounded to one user, bounded in token lifetime. They cannot extract refresh tokens (not in the env), cannot act as any other app or owner (the bearer is app-scoped and identity is derived from it), and cannot mint for other envs. They also cannot extract historical access tokens — the broker never persists them.
