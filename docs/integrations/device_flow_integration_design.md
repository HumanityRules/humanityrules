...
- WebUI `humr-integrations.js`: `connect_mode === 'device'` → `startDeviceConnect` (a code+link dialog that polls the provider-keyed status route). Disconnect reuses the unified TLS path.
