...
- **Left sidebar (fixed, 288px):** DevOps Hero logo at top, organization selector dropdown ("Course Hero" in demo data), then vertical nav links: Dashboard, Workspaces, Environments, Security, Settings. Each has an icon. Active page is highlighted.
...
- **"Deployed to Environments" section:** Table showing each environment where this app is live:
  ...
  - AWS Account info (e.g., "CH Sandbox"), Region (us-east-1), Git ref
...
- **Grid of environment cards** (3 per row). Each card shows:
  ...
  - **Account:** AWS account friendly name (e.g., "CH Sandbox")
...
- **User list:** Each row shows:
  - **Email** (e.g., amy.king@coursehero.com)
...
- **Current Organization section:**
  ...
  - Table: Organization name (e.g., "Course Hero").
...
- **Organization section:**
  ...
  - Table: Name ("Course Hero"), Slug ("course-hero").
...
- **Table:** Name, AWS Account ID, Status, Created, Edit link.
  - Example row: "CH Sandbox" | 266117665083 | green "Connected" | Jan 23, 2026 | Edit
...
- **Two-panel layout:**
  ...
  - **Right panel (sidebar, 384px):** Scrollable conversation list.
    ...
    - **Conversation items:** Each shows:
      - **Title** (auto-generated or default "New Conversation", e.g., "Simple Dashboard Deployment Setup", "DB Portal Deployment Blueprint Recreation", "AWS CH Sandbox Environment Setup")
