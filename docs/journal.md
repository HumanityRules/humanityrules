...
Three compounding issues surfaced on `hermes-vmendi00` after the v2026.4.23 agent bump and the WebUI patches landed: skills panel was empty despite hermes-agent shipping 72 bundled skills, the Bedrock model dropdown was showing ~10 models instead of the 3 we curate, and — discovered along the way — two of the three existing `patches-webui/*.patch` files had silently broken hunk headers that were only working by luck. Fixed all three in one pass, plus hardened the Dockerfile so future drift fails loud.

**Skills panel was empty because upstream never seeds `$HERMES_HOME/skills/` outside of CLI flows.** Upstream `hermes-agent/skills/` ships 72 curated skills (architecture-diagram, ascii-art, claude-code, dogfood, hermes-agent, opencode, and dozens more). `_find_all_skills()` in `tools/skills_tool.py` scans `$HERMES_HOME/skills/` and config-driven `skills.external_dirs`, never the bundled `hermes-agent/skills/` tree directly. Upstream bridges this gap with `tools.skills_sync.sync_skills()`, which copies the bundled tree into `$HERMES_HOME/skills/` and tracks origin hashes in `$HERMES_HOME/skills/.bundled_manifest` so re-runs are idempotent and don't clobber user edits. That function runs during `hermes update` (agent CLI) and `hermes profile create` (profile setup) — two flows our ECS deploy never goes through. Result: `~/.hermes/skills/` stayed empty forever.

Added a subprocess invocation in `entrypoint.sh` right after `patches/apply.py`. Runs `python3 -c "from tools.skills_sync import sync_skills; ..."` with `HERMES_HOME` pointed at the EFS dir. Uses the system python3 (same interpreter `apply.py` uses) — verified that `skills_sync.py` has only stdlib imports plus `hermes_constants`, so no venv needed. Idempotent: after first run the manifest makes every subsequent boot a cheap hash-check (<1s for 72 skills). Live-tested: one run copied all 72 skills; second run returned `copied=0, updated=0, skipped=72` (the expected steady state). Wrapped in `|| echo "... sync failed (non-fatal)"` so a skills seeding failure doesn't brick container boot.

**The browser-side "Error: Failed to fetch" was a symptom of the empty directory, not a real fetch error.** Before the seed ran, `/api/skills` was triggering `skills_list()` which would auto-`mkdir` the directory on first call and return an empty list — but the window where the endpoint was called before the mkdir completed (or when the user hit it repeatedly) produced transient failures the browser displayed as "Failed to fetch." Once any call succeeded, the directory persisted and subsequent calls returned a clean empty list. The `Failed to load session` error was unrelated: browser localStorage remembered a session UUID from a previous container lifecycle, and the fresh EFS seed had no matching session.

**The Bedrock dropdown bloat came from a second endpoint the UI hits.** `/api/models` (static) correctly returned only our three curated entries from `config.yaml`'s `providers.bedrock.models`. But the WebUI *also* calls `/api/models/live` and merges the results — a feature upstream added in v0.50.152 (#872) specifically so Bedrock/OpenRouter/Anthropic users see models as they enable them in their accounts. For Bedrock that delegates to `hermes_cli.models.provider_model_ids("bedrock")` which under the hood calls `discover_bedrock_models(region)` in `agent/bedrock_adapter.py`, hitting AWS's `ListFoundationModels` + `ListInferenceProfiles` APIs. Our CH Sandbox task role has access to ~10 Bedrock models (the three we want plus Opus 4.6-v1, Sonnet 4.5, Nova Pro/Lite/Micro, DeepSeek v3.2, and Llama 4 Maverick/Scout), so those all landed in the dropdown alongside our three. The merge is deterministic — `_addLiveModelsToSelect()` in `static/ui.js` normalizes IDs and dedupes against the static list — but there's no upstream knob to *disable* live discovery per-provider, only per-call-site.

Considered three fix paths:

- **IAM denial.** Deny `bedrock:ListFoundationModels` + `bedrock:ListInferenceProfiles` on the ECS task role. Discovery silently returns `[]`, merge becomes a no-op. Zero code, purely at the permission boundary. Rejected because it hides the intent — anyone later seeing "discovery returns empty" would have to chase IAM to understand why.
- **Agent patch.** Stub `discover_bedrock_models()` in `bedrock_adapter.py` to return `[]`. Catches any caller, not just the WebUI path. Rejected because it's invasive and could break CLI flows (`hermes models`) where discovery is actually wanted.
- **WebUI patch.** Return empty list from `_handle_live_models` when `provider == "bedrock"`. Scoped to the exact call site that was causing the bloat, expresses intent declaratively. Picked this one.

Patch is `03-bedrock-skip-live-discovery.patch`: 6-line addition that short-circuits `_handle_live_models` after alias normalization. Other providers (OpenRouter, Anthropic, Copilot) keep their live discovery — DOH deployments on those genuinely want account-available models (no curated static list exists). Only Bedrock opts out because we own the curated list server-side.

**The patch-header accounting trap — patches 01 and 02 had been wrong the whole time.** While verifying patch 03, I tried applying it on the live container (GNU patch 2.8) after it applied cleanly on my laptop (Apple `patch 2.0-12u11`). Got `Hunk #1 FAILED at 2498`. Identical source file (SHA256 match), identical patch file (SHA256 match), different outcomes. Root cause: I'd been hand-counting hunk line numbers in the `@@` headers and getting them subtly wrong. Apple `patch` is lenient — it re-scans and matches by context anyway. GNU `patch` is strict about the advertised line counts. Worse: when I ran all three existing patches through GNU `patch` on Linux in a docker run, **01 and 02 were applying "with fuzz 2"** — meaning GNU patch was tolerating context mismatches via its default fuzz factor, and they'd been working by luck. A stricter config (`--fuzz 0`) or a future WebUI bump that shifted context lines could have silently broken them.

Fix: regenerated all three patches using `diff -u` from Python-applied edits instead of hand-rolling headers. `diff -u` gets the line-count arithmetic exact because it's derived from actual file diffs, not mentally counted. Verified via `docker run alpine patch -F 0` (zero fuzz tolerance): all three apply clean. Also hardened the Dockerfile's `patch` invocation to use `-F 0`, so any future drift fails the build loudly instead of silently accumulating misalignment.

**Why the `-F 0` hardening is non-optional going forward.** Fuzz-accepted patches have an insidious failure mode: they apply somewhere *near* the intended location but not *at* it. If the context around the target has shifted but the hunk still finds a near-match, GNU patch applies the change at the near-match location. On a WebUI bump where upstream refactored the surrounding code, this could mean our bedrock-skip check lands in a different branch, or our 200-log filter guards a different method. `-F 0` forces us to notice the context mismatch at image-build time (where we can re-anchor the patch correctly) instead of at runtime (where symptoms would be bizarre).

**The live container was fixed in-place during debugging.** Because I called `sync_skills()` from inside the container to diagnose the skills issue, the running `hermes-vmendi00` already has 72 skills seeded. The curated Bedrock dropdown fix requires a redeploy since it's a source patch that needs to land during image build and `__pycache__` regen. The skills seeding also needs a redeploy for *new* apps to benefit, but existing apps with populated `~/.hermes/skills/` stay fixed across restarts.

**Revisit the hermes-agent patches too.** The same hand-rolled-header pattern was used for the agent-side patches (`patches/*.patch`), which apply at boot via `patches/apply.py`, not at build time. `apply.py` uses `patch -p1 -N --dry-run` followed by `patch -p1 -N` — no `-F` flag. If those patches also have soft headers, they could silently drift too. Didn't audit them in this session because they're actively known to work in production; worth a follow-up verification run before the next hermes-agent bump.

**Key points:**

- WebUI skills panel was empty because upstream's `sync_skills()` is only wired into CLI flows (`hermes update`, `hermes profile create`), neither of which our ECS deploys execute. Added a subprocess call in `entrypoint.sh` right after `apply.py` that runs `sync_skills()` idempotently on every boot. Uses system python3 (no venv needed — skills_sync has only stdlib + hermes_constants imports).
- Bedrock model dropdown was showing ~10 models because the WebUI has a *second* endpoint (`/api/models/live`) that merges live-discovered models into the dropdown. For Bedrock, live discovery hits AWS's `ListFoundationModels`/`ListInferenceProfiles` and returns whatever the task role can see. Patch 03 short-circuits the endpoint for `provider == "bedrock"` so only our curated static list reaches the UI.
- Rejected IAM denial and agent-side stubbing in favor of a targeted WebUI patch. IAM hides intent; agent stub could break CLI flows that legitimately want discovery. Patch scopes the suppression to exactly one call site.
- Discovered patches 01 and 02 had been applying with `fuzz 2` on GNU patch — working by luck via default fuzz tolerance, silently wrong hunk headers. Regenerated all three from `diff -u` output so line counts come from actual diffs, not manual counting. Verified on Linux GNU patch 2.8 with `-F 0` (zero tolerance).
- Hardened Dockerfile to use `patch -F 0` so future drift fails the build instead of silently applying misaligned.
- Live container already has the skills fix from diagnostic calls; dropdown fix needs a redeploy to take effect. Agent-side `patches/*.patch` files weren't audited this session; worth a verification pass before the next hermes-agent bump since they follow the same hand-rolled-header pattern.
- "Error: Failed to fetch" in the skills panel was a transient symptom of the empty directory (race between the UI's fetch and the server's lazy `mkdir` in `skills_list()`). "Failed to load session" was browser localStorage pointing at a session UUID that didn't exist in the fresh EFS seed. Neither was a real network/CORS failure.
...
Bumped both Hermes pins in `template_repos/hermes_agent/Dockerfile`: WebUI `0.50.126 → 0.50.236` (148 commits upstream) and agent `v2026.4.16 → v2026.4.23` (upstream release 2026-04-23, "The Interface release — new Ink TUI, pluggable transport architecture, native AWS Bedrock, five new inference paths, GPT-5.5 via Codex OAuth, /steer mid-run nudges, …"). `v2026.4.23` is currently the top tag; `main` is 855 commits further ahead. With the agent bump, three of the four DOH-owned patches became unnecessary and were deleted. One patch and the `overlay/agent/bedrock_aux_client.py` file still carry their weight against `v2026.4.23` and stayed in the tree; they can both be dropped the day we decide to track `main`.

**Patch-by-patch audit vs. `v2026.4.23`.** The previous pin carried four numbered patches plus one overlay file, all of them Bedrock-related or diagnostic:

- **`01-run_agent-bedrock-dots.patch` — dropped, landed-generalised.** Our one-line addition of `"bedrock"` to the `_anthropic_preserve_dots()` provider allowlist is now redundant: upstream `run_agent.py:6798-6827` ships the full allowlist (`alibaba, minimax, minimax-cn, opencode-go, opencode-zen, zai, bedrock`) and additionally matches `"bedrock-runtime."` in the base URL as defense-in-depth for the case where `provider` is unset. This is the path that prevents AWS's dotted inference-profile IDs (`us.anthropic.claude-opus-4-7`, `us.anthropic.claude-sonnet-4-5-20250929-v1:0`) from being hyphen-normalised and rejected with `HTTP 400 The provided model identifier is invalid` — confirmed still wired end-to-end by asking the live deployment "what model are you?" and getting "Opus 4.7 via Bedrock" back.
- **`02-run_agent-bedrock-caching.patch` — dropped, landed-generalised.** All three `is_native_anthropic = self.api_mode == "anthropic_messages" and self.provider == "anthropic"` sites that our patch rewrote to `provider in {"anthropic", "bedrock"}` are gone. Upstream collapsed them into a single helper `_anthropic_prompt_cache_policy()` at `run_agent.py:2384-2456` that returns `(should_cache, use_native_layout)` and covers Bedrock+Claude via the general branch *`is_anthropic_wire and is_claude`* → `(True, True)`. Bedrock+Claude also now goes through `AnthropicBedrock` SDK directly for the main path (`run_agent.py:1099-1113`), so native-layout `cache_control` markers flow through with no DOH involvement. This is the cleanest of the three "landed" cases — a scattered three-site conditional became one helper, which is also a better extension point if we ever need per-app cache policy overrides.
- **`03-auxiliary_client-wire-bedrock.patch` — kept for `v2026.4.23`, will drop when we move to `main`.** Upstream `v2026.4.23`'s `agent/auxiliary_client.py` still has **zero** references to `bedrock` or `aws_sdk` — the `resolve_provider_client()` handler for `pconfig.auth_type == "aws_sdk"` only exists on `main` (at line 2313, where it wraps a real `AnthropicBedrock` client in `AnthropicAuxiliaryClient` with base_url `bedrock-runtime.<region>.amazonaws.com`). Without this patch, every auxiliary call — vision, compression, session_search, skills_hub, approval, mcp, flush_memories — silently returns `None` on Bedrock deployments and the feature is unavailable. Line anchors drifted (our hunks were at 1293/1668; upstream is now at 1500/1892) but the context lines the patch pins on still exist verbatim; verified by cloning `v2026.4.23`, copying the overlay in, and running `patch -p1 -N --dry-run` and then the real apply — clean, no fuzz, no rejects. When we eventually move to `main` we can delete both this patch and the `overlay/agent/bedrock_aux_client.py` file, since upstream's native wiring covers the same surface.
- **`04-docker-cleanup-traceback.patch` — deleted per user instruction.** This was a DOH-only diagnostic that logged a `traceback.format_stack()` every time `DockerEnvironment.cleanup()` fired, to help us identify the callers tearing down the tool container and breaking snapshot continuity. The user confirmed this is no longer needed — we've since landed a different `04-docker-persistent-cleanup-detach.patch` (untracked, coexisting in the tree) that actually changes the cleanup behavior instead of just observing it, so the traceback was redundant.

**Overlay file `overlay/agent/bedrock_aux_client.py` — kept unchanged.** Upstream still does not ship a `bedrock_aux_client.py` at `v2026.4.23` (`gh api contents/agent/bedrock_aux_client.py?ref=v2026.4.23` returns 404). Our overlay provides the `BedrockAuxiliaryClient` class that patch 03 imports; removing the overlay would break patch 03's import. The two live and die together.

**WebUI bump is a pure image tag change.** `FROM ghcr.io/nesquena/hermes-webui:0.50.126 → 0.50.236`. The WebUI is not patched by DOH and has no DOH overlay; the only risk surface is whatever ships in those 148 upstream commits. The release note for the agent covers the cross-cutting changes (new transport layer, Bedrock, five new providers) but those are agent-side; the WebUI bump in lockstep is partly just to stay on a supported combo and partly to pick up dashboard plugin system + theme switching + auxiliary models config UI mentioned in the release. Deliberately did not scan WebUI diff in detail — if something breaks at runtime, the fallback is pinning back to 0.50.126 and bisecting on a `v0.50.x` tag.

**Background upstream changes worth flagging for future work.** The agent bump brings a few things we don't use yet but might want:

- **New `api_mode` `"bedrock_converse"`** for non-Claude Bedrock models (Nova, Llama, Mistral via Converse API). Auto-detected from `provider == "bedrock"` or `bedrock-runtime.<region>.amazonaws.com` base URL at `run_agent.py:842,865-871`. Coexists with `"anthropic_messages"` for Claude-on-Bedrock via `AnthropicBedrock`. Opens the door to non-Claude Bedrock models without a config change on our side — today we only ship Claude inference-profile IDs in the WebUI dropdown (see follow-up bug below).
- **Bedrock Guardrails** plumbed via `config.yaml` `bedrock.guardrail.*` (identifier, version, stream_processing_mode, trace), read at init and applied to every Converse call (`run_agent.py:1146-1165`). Might be worth exposing via `DOH_BEDROCK_GUARDRAIL_*` runtime vars later; not doing it now because no customer has asked.
- **`PROVIDER_REGISTRY` gained `"bedrock"` with `auth_type="aws_sdk"`** and aliases `aws / aws-bedrock / amazon-bedrock / amazon` (`hermes_cli/auth.py:319-323,1036`). The `"auto"` provider-selection chain now probes `has_aws_credentials()` for implicit Bedrock selection — i.e. in an ECS task with IAM credentials and no other provider env vars, `provider: auto` would now resolve to Bedrock. We set `provider: bedrock` explicitly in `config.yaml` so this doesn't change our behavior, but it's useful to know the auto path exists.
- **Five new inference paths** (NVIDIA NIM, Arcee, Step Plan, Gemini CLI OAuth, ai-gateway) and GPT-5.5 via Codex OAuth. Not wired through DOH yet.

**Verification.** Three things had to hold for the bump to be safe:

1. Patch 03 applies cleanly against a fresh `v2026.4.23` checkout with the overlay copied in. Checked by cloning the tag into `/tmp/hermes-check/`, copying `overlay/agent/bedrock_aux_client.py` into place, and running both `patch -p1 -N --dry-run` and the real apply — no fuzz, no rejects, "patching file agent/auxiliary_client.py" single-line output both times.
2. Bedrock main-agent path still works end-to-end on a running deployment. Verified against the `hermes-vmendi00` app: the in-WebUI profile shows `Model us.anthropic.claude-opus-4-7, Provider bedrock`, and asking the model to identify itself returns "Opus 4.7 via Bedrock". So the agent is correctly picking up Bedrock from `config.yaml`, resolving IAM credentials via IMDS (`has_aws_credentials()` → boto3's default chain), and routing through `AnthropicBedrock` SDK.
3. Patches apply idempotently on existing EFS-backed deployments. `apply.py` uses `patch -p1 -N --dry-run` first and only applies if dry-run succeeds, distinguishing "already applied" from "real failure" via the "previously applied" marker in the output. The three deleted patches become absent, not reversed — for existing EFS volumes their previous application stays intact until a full re-seed, which is the correct behavior (the fix is no-ops now anyway).

**One follow-up bug identified, not yet fixed.** The WebUI model dropdown on Bedrock deployments shows GPT-5.4 instead of Bedrock models — `hermes-vmendi00` has the agent correctly running on Bedrock/Opus 4.7, but the WebUI profile dropdown only surfaces OpenAI models. Traced to the WebUI's group-builder path in `api/config.py:1685`: `_PROVIDER_MODELS` (the hardcoded model catalog at `api/config.py:612-758`) has entries for anthropic, openai, google, deepseek, nous, zai, kimi-coding, minimax, copilot, opencode-zen, opencode-go, gemini, mistralai, qwen, x-ai — but no `bedrock` entry. Our `config.yaml.template` ships `providers: {}`, so the `providers.bedrock.models` fallback at `api/config.py:1685-1694` doesn't fire either. `bedrock` is in `_PROVIDER_ALIASES` (lines 578-581), but that table only handles alias resolution, not dropdown population. Net effect: bedrock ends up in the `else` branch which only appends a group if `auto_detected_models` is non-empty; since `bedrock-runtime.<region>.amazonaws.com/v1/models` doesn't exist, no group is added and only the OpenAI-keyed providers show up. The agent is unaffected because it reads `model.provider: bedrock` from `config.yaml` directly. Fix will be to populate `providers.bedrock.models` in `config.yaml.template` when `DOH_LLM_PROVIDER=bedrock`, with a static list of inference-profile IDs (Claude Opus 4.7, Sonnet, Haiku) — simpler than teaching the WebUI to call `discover_bedrock_models()`, which exists in `hermes_cli/models.py:2625-2629` but isn't wired into the WebUI's dropdown flow. Left as a separate change to keep this commit a pure version bump.

**Key points:**

- Agent pin `v2026.4.16 → v2026.4.23`, WebUI pin `0.50.126 → 0.50.236`; both in `template_repos/hermes_agent/Dockerfile`.
- Deleted patches 01 and 02 because the underlying Bedrock fixes landed upstream — generalised into a provider allowlist and a new `_anthropic_prompt_cache_policy()` helper respectively.
- Deleted patch 04 at user direction: the diagnostic traceback is no longer needed now that a replacement patch actually fixes the persistent-cleanup path.
- Kept patch 03 and its overlay: upstream's `agent/auxiliary_client.py` on `v2026.4.23` still has no Bedrock handler; without them, all auxiliary LLM features silently return `None` on Bedrock deployments.
- Verified patch 03 applies cleanly against `v2026.4.23` + overlay with `patch -p1 -N --dry-run` and real apply — no fuzz, no rejects.
- Verified agent path by querying the live `hermes-vmendi00` deployment: profile shows Opus 4.7/bedrock, self-identification confirms "Opus 4.7 via Bedrock".
- Follow-up: WebUI Bedrock model dropdown is empty because `_PROVIDER_MODELS` has no `bedrock` entry and `config.yaml` ships empty `providers`. Will be fixed by populating `providers.bedrock.models` in the template when `DOH_LLM_PROVIDER=bedrock`. Not in this commit.
- When we eventually unpin from `v2026.4.23` onto `main` (or a future tag that includes upstream's `aws_sdk` auth_type handler at `auxiliary_client.py:2313`), patch 03 and the overlay can be deleted entirely — upstream's native wiring covers the same surface.
...
Deployed `hermes-slack`, the agent said "I don't have any tools that start with `mcp_`" — no `mcp_learneo_*` tools offered to the LLM. `config.yaml` had `mcp_servers.learneo`, the `learneo-mcp` sibling container answered MCP `initialize` fine, the hermes toolset aliases correctly resolved `learneo` → `mcp-learneo` (10 tools). End-to-end wiring was correct *in a fresh Python process*. Only the long-lived WebUI process didn't see the tools.
...
- *"MCP toolset name mismatch."* Registry stores tools under canonical `mcp-learneo` but we pass `learneo` in `enabled_toolsets`. Resolver *does* follow alias chain (`validate_toolset` returns True for alias names; `get_toolset` calls `get_toolset_alias_target`). Verified via direct probe: `get_tool_definitions(enabled_toolsets=["learneo"])` returns 10 `mcp_learneo_*` schemas in a fresh process. Wiring is fine.
...
**2. learneo-mcp container exited 0 before Hermes started → TaskFailedToStart.** Every stopped task showed the same pattern: `learneo-mcp exitCode=0, hermes exitCode=null`. Because ECS containers are `essential=True` by default, the MCP's clean exit killed the pod before Hermes's first boot. Root cause: learneo-mcp's `ENTRYPOINT ["node", "dist/index.js"]` with no CMD, so without the `--http` flag `src/index.ts:91` fell through to `startStdioServer()` which blocks on stdin. ECS Fargate tasks have no attached TTY, so stdin hits EOF immediately, the server exits 0, and the task is killed.

Fix required a new per-container field `command: list[str] | None` on `ContainerConfig`, plumbed through `app_config_builder.py` to `task_definition.add_container(command=…)`. The image's `ENTRYPOINT` is preserved; only the CMD is overridden. Seeded the learneo-mcp container with `command: ["--http", "--port", "7777", "--host", "127.0.0.1"]`. This is also a **generic mechanism** — any future prebuilt-image AppTemplate where the image's default CMD doesn't match how DOH wants to run it can now set this.

Initially added a `curl -fsS http://127.0.0.1:7777/health` healthcheck too, which was wrong on three counts: (a) learneo-mcp's HTTP transport only routes `/mcp` — `/health` would 404 every probe, (b) `node:22-slim` doesn't ship curl, and (c) coupling the MCP's liveness to the task's health via `essential=True` reintroduced the blast-radius tradeoff we'd explicitly rejected earlier. Dropped the healthcheck entirely and added another new per-container field `essential: bool = True` (defaults to ECS default), setting the learneo-mcp container to `essential=False`. Now an MCP crash leaves the task up; Hermes gracefully degrades to a pure-LLM state, and the MCP gets restarted on the next task redeploy.
...
- **Per-container `command` and `essential` are reusable now.** Not just learneo-mcp-specific — any prebuilt container whose image was built for a different runtime model (stdio CLI, foreground daemon, etc.) can override its CMD; any sidecar whose crash shouldn't kill the task can set `essential=False`. Both map 1:1 to ECS primitives.
...
- `depends_on`. ECS default is "start in parallel, no coupling," which matches the product decision: Hermes must start regardless of learneo-mcp's health; the MCP client reconnects if the aggregator comes up later. Adding `depends_on` would earn no behavior today.
...
**Hermes config plumbing.** `config.yaml.template` ends with a static `mcp_servers.learneo: { url: "http://127.0.0.1:7777/mcp", enabled: true, timeout: 120 }` block. No auth header — loopback is the boundary. Hermes's entrypoint (`entrypoint.sh:49`) only writes `config.yaml` from the template on first boot, so for the greenfield workspace new deploys pick this up automatically; any future change to the block requires the documented EFS `config.yaml` regeneration step.

**The learneo-mcp container's upstream credential fields** (GITHUB_TOKEN, JIRA_API_TOKEN, ATLASSIAN_EMAIL, ATLASSIAN_API_TOKEN, CONFLUENCE_API_TOKEN, DATADOG_API_KEY, DATADOG_APP_KEY, GROUNDCOVER_API_KEY, CONTROLMONKEY_API_TOKEN, CLOUDFLARE_API_TOKEN, GITLAB_TOKEN) are seeded with `value=""`, which triggers the fall-through to `devopshero/{env_slug}/shared-secrets`. Ops populates that bundle once per env; every Hermes in the env inherits everything automatically. Known consequence accepted up front: the aggregator's capabilities are bounded by the lowest-privilege tenant in the env. Acceptable for a pre-beta shared Slack bot with read-leaning service-account scopes. Per-user credentials would require the rejected sidecar-per-task shape; that door stays closed for now.
...
- **Per-container logs are a visible win.** Log stream prefix is `{app_slug}-{container_name}`, so `doh_app_logs` operators see `my-hermes-hermes` vs `my-hermes-learneo-mcp` streams separately. No tooling change needed; the log driver config is per-container already.
...
- **Operator step before first Hermes Slack deploy:** build and push learneo-mcp with `uv run manage.py doh_build_prebuilt_image --account <acct> --source-dir ../learneo-mcp --ecr-repo learneo-mcp --tag 0.1.0`. `LEARNEO_MCP_IMAGE_VERSION` in `seed_app_templates.py` must match the `--tag` used. Then `seed_app_templates` + deploy.
...
The app detail page has two stacked sections: "Deployed to Environments" (one summary row per env) and "Recent Deployments" (chronological list). After tearing an app down, the summary row still showed a green "Succeeded" pill with a 15-hour-old timestamp, while the list below correctly showed "Torn Down" at the top. Bug report from looking at the Hermes deploy in CH Sandbox.
...
- **Okta cardinality for v1: one app per org; restrict to single-env orgs.** Multiple envs would each need their own callback URI and thus their own Okta app. Deferred the `EnvironmentOIDCConfig` table; CH Sandbox is single-env, so this doesn't block the pilot.
...
- **ALB priority 10 has not yet been audited against existing hashed app priorities.** Worth a sanity check against CH Sandbox state before first deploy.
...
- **`bootstrap_organization` is now the single place that installs the PA policy** — new orgs get it automatically; existing orgs (like course-hero in dev) need a one-shot backfill. Did this manually during verification.
...
- **CDK synth passed locally with the Lambda bundling image actually running pip** — good signal the AWS deploy will work on first attempt, though real verification is pending CH Sandbox deploy.
...
- Debugging approach: assumed role into customer AWS account (CH Sandbox 266117665083) via the stored `role_arn`/`external_id` on `AWSAccount` model, then inspected ECS events, stopped task descriptions, and CloudWatch container logs to reconstruct the timeline.
...
- **`seed_test_users` made reusable** — Added `--email-domain` parameter (default: `coursehero.com`) so the command can be called for any org. `seed_prepare_demo` calls it with `meridiansystems.com` to populate Meridian's People page.
