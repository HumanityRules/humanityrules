...
Resulting names (example: org=`humr`, env=`sandbox`, app=`humr-slack`, rid=`9f3a2b7c1d4e`):

- **IAM task role** (limit 64): `bounded_app_name(app, rid, limit=64, suffix="-task-role")`
  → `humr-humr-slack-9f3a2b7c1d4e-task-role` — **full app slug fits.**
- **Aurora cluster identifier** (limit 63): `bounded_app_name(app, rid, limit=63, suffix="-aurora")`
  → `humr-humr-slack-9f3a2b7c1d4e-aurora` — **full app slug fits.**
- **Target group** (limit 32): `bounded_app_name(app, rid, limit=32)`
  → `humr-humr-sl-9f3a2b7c1d4e` — app trimmed to ~14 chars (limit is tight; unavoidable).
