...
#   ./prod_manage.sh humr_control create-env --aws-account "Humanity Rules AWS Account" --name default --region us-east-1 --hosted-zone humanityrules.io --provision
#   ./prod_manage.sh shell
#   ./prod_manage.sh dbshell
#
# Examples (local-exec, target resolved from prod):
#   ./prod_manage.sh humr_build_prebuilt_image --account "Humanity Rules Sandbox" --env production --source-dir template_repos/humr_dind --ecr-repo humr-dind --tag 0.2.5
#
set -e
...
if [ $# -eq 0 ]; then
    ...
    echo "  ./prod_manage.sh humr_build_prebuilt_image --account \"Humanity Rules Sandbox\" --env production --source-dir template_repos/humr_dind --ecr-repo humr-dind --tag 0.2.5"
    exit 1
fi
