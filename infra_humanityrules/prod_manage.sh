...
#   ./prod_manage.sh humr_build_prebuilt_image --account "Humanity Rules Sandbox" --env production --source-dir template_repos/humr_dind --ecr-repo doh-dind --tag 0.2.5
#
set -e
...
if [ $# -eq 0 ]; then
    ...
    echo "  ./prod_manage.sh humr_build_prebuilt_image --account \"Humanity Rules Sandbox\" --env production --source-dir template_repos/humr_dind --ecr-repo doh-dind --tag 0.2.5"
    exit 1
fi
