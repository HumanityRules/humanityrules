...
from .abac_helpers import require_org_admin
...
@login_required
def security_people(request):
    """List organization members with their effective attributes."""
    if not request.htmx:
        context = get_app_shell_context(request=request, current_page="security")
        context["content_url"] = "/security/people/"
        return render(request, "devopshero_app/app_shell.html", context=context)

    denied = require_org_admin(request)
    if denied:
        return denied
...
@login_required
def security_people_detail(request, user_id):
...
@login_required
@require_POST
def security_people_attribute_add(request, user_id):
...
@login_required
@require_POST
def security_people_attribute_remove(request, user_id, attribute_id):
...
@login_required
@require_POST
def security_people_group_add(request, user_id):
...
@login_required
@require_POST
def security_people_group_remove(request, user_id, membership_id):
...
def _render_people_attributes_partial(request, org, member):
...
@login_required
def security_group_detail(request, group_id):
...
@login_required
@require_POST
def security_group_delete(request, group_id):
    """Delete a group."""
    denied = require_org_admin(request)
    if denied:
        return denied
...
@login_required
@require_POST
def security_group_attribute_add(request, group_id):
...
@login_required
@require_POST
def security_group_attribute_remove(request, group_id, attribute_id):
...
@login_required
@require_POST
def security_group_member_add(request, group_id):
    """Add a member to a group."""
    denied = require_org_admin(request)
    if denied:
        return denied
...
@login_required
@require_POST
def security_group_member_remove(request, group_id, membership_id):
    """Remove a member from a group."""
    denied = require_org_admin(request)
    if denied:
        return denied
...
def _render_group_attributes_partial(request, group):
    attributes = group.attributes.order_by("key", "value")
...
def _render_group_members_partial(request, org, group):
    members = group.memberships.select_related("user").order_by("user__email")
    member_user_ids = members.values_list("user_id", flat=True)
    available_users = User.objects.filter(
        organization_memberships__organization=org,
...
@login_required
def security_policy_detail(request, policy_id):
...
@login_required
@require_POST
def security_policy_delete(request, policy_id):
    """Delete a policy."""
    denied = require_org_admin(request)
    if denied:
        return denied
...
def _get_existing_tag_keys(org):
    """Get distinct resource tag keys and identity attribute keys for datalist suggestions."""
    ...
    return sorted(tag_keys | attr_keys | group_attr_keys | {"org-role", "authenticated"})


def _get_existing_tag_values(org):
    """Get distinct resource tag values and identity attribute values for datalist suggestions."""
    ...
    return sorted(tag_vals | attr_vals | group_attr_vals | {"admin", "true"})
