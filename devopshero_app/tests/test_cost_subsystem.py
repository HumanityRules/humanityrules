"""Tests for the per-app cost subsystem (devopshero_app/services/cost/).
...
from devopshero_app.services.cost import bedrock_pricing as pricing
from devopshero_app.services.cost import cost_refresh, panel
from devopshero_app.services.cost.cost_source import DailyCostRow
...
class TestCostPanel(_CostFixtureMixin, TestCase):
    ...
    def test_chart_fragment_template_renders(self) -> None:
        ...
        CostRefreshJob.objects.create(organization=self.org, app=self.app, status=CostRefreshJob.Status.PENDING)
        context = panel.build_panel_context(app=self.app)
        html = render_to_string("devopshero_app/apps/_app_cost_chart.html", context)
