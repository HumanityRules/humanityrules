...
Used by the sidecar end-to-end test (see sidecar_e2e_test) and for manual browser
testing: paste the printed token into a doh_session cookie on the deployed app's
parent domain and the sidecar will accept it (within TTL) without going through
Okta.
...
Usage:
    uv run manage.py sidecar_mint_cookie \\
        --aws-account "Humanity Rules Sandbox" --env-slug sidecar-e2e \\
...
def mint_session_jwt(
    ...
    try:
        ...
        raise CommandError(
            f"Secret {secret_name!r} not found. Has a sidecar-enabled app been deployed to this env?",
        ) from exc
