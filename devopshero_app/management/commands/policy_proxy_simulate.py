"""
Simulate the policy proxy's decision pipeline against a local DOH instance.

Bypasses AWS entirely: generates an in-memory RSA keypair, mints a session JWT
the way the auth Lambda will, ensures a PolicyProxyToken row for the app's env,
...
deploying the policy proxy or the auth Lambda.

Example:
    uv run manage.py runserver   # in another terminal
    uv run manage.py policy_proxy_simulate --app vmendi-hermes --user vmendi
"""
...
from devopshero_app.models import App, DeploymentBlueprint, PolicyProxyToken, User


class Command(BaseCommand):
    help = "Simulate a policy-proxy decision against the local PDP endpoint (no AWS required)."
    ...
    def handle(self, *args, **opts):
        ...
        raw_token, token_row = _ensure_policy_proxy_token(environment=environment)
        ...
        self.stdout.write(self.style.MIGRATE_HEADING("Simulating policy-proxy decision"))
...
def _ensure_policy_proxy_token(environment) -> tuple[str, PolicyProxyToken | None]:
    """Return (raw_token, existing_row).

    If a PolicyProxyToken row already exists for this env we can't recover the raw
    ...
    existing = PolicyProxyToken.objects.filter(environment=environment).first()
    if existing is not None:
        existing.token_hash = token_hash
        existing.save(update_fields=["token_hash"])
        return raw, existing
    PolicyProxyToken.objects.create(environment=environment, token_hash=token_hash)
    return raw, None
