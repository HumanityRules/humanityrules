defmodule Periscope do
  ...
  Taken from https://github.com/caleb-bb/periscope, but patched to work
  with the latest liveview.  The official Periscope does not find any
  liveview processes because of changes in the initial_call property (as of
  095ef66)

  Also modified `component_states` to exclude current PID.  If not calling
  from a liveview then no change.  if calling from a liveview then
  prevents an error when :sys.get_state(pid) tries to send a msg to itself.
  """
  ...
  defp component_states do
    liveview_pids()
    |> Enum.reject(& &1 == self()) # don't send msg to self (error)
