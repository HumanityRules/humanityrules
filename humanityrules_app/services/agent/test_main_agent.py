...
Usage:
  ...
  uv run python -m humanityrules_app.services.agent.test_main_agent --prompt "Deploy"

  # Customize workspace and repo context
  uv run python -m humanityrules_app.services.agent.test_main_agent --workspace myws --repo acme/flask-api --prompt "Deploy"

  # General mode with no context
  uv run python -m humanityrules_app.services.agent.test_main_agent --no-context --prompt "Hello"

  # Override mode explicitly
  uv run python -m humanityrules_app.services.agent.test_main_agent --mode general --prompt "Hello"

  # Fork from a conversation (default when --conversation-id provided)
  uv run python -m humanityrules_app.services.agent.test_main_agent --conversation-id <uuid> --prompt "Try this"

  # Resume a conversation in place
  uv run python -m humanityrules_app.services.agent.test_main_agent --conversation-id <uuid> --no-fork --prompt "Continue"

  # Interactive REPL mode
  uv run python -m humanityrules_app.services.agent.test_main_agent --repl
...
def _format_tool_label(tool_name: str, parameters: dict[str, Any]) -> str:
    import humanityrules_app.services.agent.mcp_tools as mcp_tools
...
async def _resolve_conversation_for_fork(user, source_id: str):
    ...
    from humanityrules_app.models import Conversation
...
async def _resolve_workspace(user, workspace_arg: str | None):
    ...
    if not workspace_arg:
        return None
    from humanityrules_app.models import Workspace
...
async def _resolve_aws_account(user, aws_account_arg: str | None):
    ...
    if not aws_account_arg:
        return None
    from humanityrules_app.models import AWSAccount
...
async def _create_new_conversation(user, workspace_id, repo_id, aws_account_id, mode: str):
    ...
    from humanityrules_app.services.agent import agent_service
...
async def _create_user_message(conversation, content: str) -> None:
    from humanityrules_app.models import Message
...
async def _stream_agent(conversation, agent, show_tool_io: bool) -> None:
    ...
    from humanityrules_app.services.agent.agent_runner import get_pending_user_message
...
async def _run(args: argparse.Namespace, run_db_path: Path) -> int:
    ...
    from humanityrules_app.services.agent.agent_service import MainAgent
