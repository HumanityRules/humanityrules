...
2. /integrations/org/github/callback/ → exchange code, list installations, redirect to picker
3. /integrations/org/github/select-installation/ (GET) → picker page
4. /integrations/org/github/select-installation/ (POST) → connect selected installation
5. GitHub App setup URL → /integrations/org/github/setup/ → connect new installation
"""
...
from humanityrules_app.models import IntegrationGitProvider, Organization, Repository
...
SELECT_INSTALLATION_URL = "/integrations/org/github/select-installation/"


@login_required
def integrations_org_github(request: HttpRequest) -> HttpResponse:
    ...
    if not request.htmx:
        ...
        context["content_url"] = "/integrations/org/github/"
    ...
    return render(request, "humanityrules_app/integrations/git_integrations.html", context=context)
...
def integrations_org_github_callback(request: HttpRequest) -> HttpResponse:
    ...
    if not code:
        ...
        return redirect("/integrations/org/github/?error=oauth_failed")
    ...
    return redirect(SELECT_INSTALLATION_URL)
...
def integrations_org_github_select_installation(request: HttpRequest) -> HttpResponse:
    ...
    if not request.htmx:
        context = base.get_app_shell_context(request=request, current_page="integrations")
        context["content_url"] = SELECT_INSTALLATION_URL
        return render(request, "humanityrules_app/app_shell.html", context=context)

    installations = request.session.get("github_installations", [])
    if not installations:
        return redirect("/integrations/org/github/")

    context = base.get_app_shell_context(request=request, current_page="integrations")
    context["installations"] = installations
    context["install_new_url"] = github_client.get_app_installation_url()
    return render(request, "humanityrules_app/github/github_select_installation.html", context=context)


def _handle_select_installation_post(request: HttpRequest, org: Organization) -> HttpResponse:
...
def _handle_setup(request: HttpRequest, org: Organization) -> HttpResponse:
...
def _connect_installation(org: Organization, installation_id: str) -> HttpResponse:
    ...
    try:
        ...
        if created:
            logger.info(
                "Created GitHub integration for org %s (installation_id=%s, account=%s)",
                org.name,
                installation_id,
                account_name,
            )
            ...
            logger.info(
                "Updated GitHub integration for org %s (installation_id=%s, account=%s)",
                org.name,
                installation_id,
                account_name,
            )
        ...
        return redirect("/integrations/org/github/?error=connection_failed")
