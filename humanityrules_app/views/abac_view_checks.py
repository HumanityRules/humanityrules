...
def check_abac(
    ...
    if not abac_service.check_action(org, request.user, resource, resource_type, action):
...
def check_abac_create(request: HttpRequest, resource_type: str, action: str) -> HttpResponse | None:
    ...
    """
    org = request.user.current_organization
    allowed = abac_service.evaluate_policies_unscoped(org, request.user, resource_type)
...
def require_org_admin(request: HttpRequest) -> HttpResponse | None:
    ...
    """
    org = request.user.current_organization
    if not abac_service.is_org_admin(org, request.user):
