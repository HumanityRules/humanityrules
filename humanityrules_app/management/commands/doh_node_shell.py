...
Usage:
    ...
    uv run manage.py doh_node_shell --account "CH Sandbox" --env default

    # List every container instance with its current tasks, then exit.
    uv run manage.py doh_node_shell --account "CH Sandbox" --env default --list

    # Shell into whichever instance is currently hosting an app's RUNNING task.
    uv run manage.py doh_node_shell --account "CH Sandbox" --env default --app hermes-vmendi00

    # Shell into a specific instance (override autodetection).
    uv run manage.py doh_node_shell --account "CH Sandbox" --env default --instance-id i-0abc...
