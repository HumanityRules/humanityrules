...
Usage:
    uv run manage.py humr_app_exec --account "CH Sandbox" --app my-app --as hermeswebui <<'EOF'
    /app/venv/bin/python -c "from tools.mcp_tool import discover_mcp_tools; print(discover_mcp_tools())"
    EOF

    uv run manage.py humr_app_exec --account "CH Sandbox" --app my-app --script-file probe.sh --format json
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options) -> None:
        ...
        service_name = f"humr-{env_slug}-{app.slug}"
