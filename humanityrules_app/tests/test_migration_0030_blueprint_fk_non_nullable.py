...
class TestMigration0030BlueprintFkNonNullable(TransactionTestCase):
    ...
    migrate_from = [("humanityrules_app", "0029_drop_app_runtime_fields")]
    migrate_to = [("humanityrules_app", "0032_alter_deployment_blueprint_non_nullable")]
    ...
    def _create_legacy_deployment(self, status: str, subdomain: str) -> str:
        Organization = self.old_apps.get_model("humanityrules_app", "Organization")
        AWSAccount = self.old_apps.get_model("humanityrules_app", "AWSAccount")
        Repository = self.old_apps.get_model("humanityrules_app", "Repository")
        Workspace = self.old_apps.get_model("humanityrules_app", "Workspace")
        App = self.old_apps.get_model("humanityrules_app", "App")
        Environment = self.old_apps.get_model("humanityrules_app", "Environment")
        Deployment = self.old_apps.get_model("humanityrules_app", "Deployment")
    ...
    def test_migration_backfills_succeeded_legacy_deployment(self) -> None:
        ...
        Deployment = self.new_apps.get_model("humanityrules_app", "Deployment")
        DeploymentBlueprint = self.new_apps.get_model("humanityrules_app", "DeploymentBlueprint")
